...
Devaulty exposes an MCP server through the backend binary, but it is not a background daemon. An MCP client must launch it explicitly and connect over stdio.

Typical setup:

1. Use the installed Devaulty backend binary, or the locally built one generated by the project.
2. Configure your MCP client to launch that binary with the `mcp` argument.
3. Keep the host process alive while the client is using the tools.
...
The exact binary path depends on your OS and installation method. In the bundled Tauri app, the backend is placed in the app resources and resolved dynamically by the desktop shell.
